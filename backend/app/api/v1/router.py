...
def _ensure_profile(db: Session) -> Profile:
    profile = db.scalar(select(Profile).where(Profile.user_id == DEFAULT_USER_ID))
...
def _trip_payload(trip: Trip, driver: Profile, vehicle: Vehicle) -> dict[str, Any]:
    return {
        "id": trip.id,
...
def list_trips(
    ...
    items = [_trip_payload(trip, profile, vehicle) for trip, profile, vehicle in rows]
    return success_response({"items": items})
...
def get_my_active_trip(db: Session = Depends(get_db)):
    ...
    return success_response(_trip_payload(trip, profile, vehicle))
...
def create_trip(payload: TripCreateRequest, db: Session = Depends(get_db)):
    ...
    return success_response(_trip_payload(trip, profile, vehicle), status_code=201)
